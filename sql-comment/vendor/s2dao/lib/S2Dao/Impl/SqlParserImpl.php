...
class SqlParserImpl implements \S2Dao\SqlParser {
    ...
    public function parse() {
        ...
        while ($this->tokenizer->next() !== \S2Dao\SqlTokenizer::EOF) {
            $this->parseToken();
        }
        return $this->pop();
    }
    ...
    protected function parseSql() {
        ...
        $node = $this->peek();
        if (($node instanceof \S2Dao\Node\IfNode || $node instanceof \S2Dao\Node\ElseNode)
                && $node->getChildSize() === 0) {
    ...
    protected function parseEnd() {
        while (\S2Dao\SqlTokenizer::EOF != $this->tokenizer->next()) {
            if ($this->tokenizer->getTokenType() === \S2Dao\SqlTokenizer::COMMENT
    ...
    protected function parseCommentBindVariable() {
        ...
        if ($s !== false && strpos($s, '(') === 0 && substr($s, -1, 1) === ')') {
            $this->peek()->addChild(new \S2Dao\Node\ParenBindVariableNode($expr));
        } else if (strpos($expr, '$') === 0) {
            $this->peek()
                    ->addChild(new \S2Dao\Node\EmbeddedValueNode(substr($expr, 1)));
        } else {
            $this->peek()->addChild(new \S2Dao\Node\BindVariableNode($expr));
        }
    }
